...
class TestJsonLogster(unittest.TestCase):
    ...
    def test_flatten_object(self):
        flattened = self.json_logster.flatten_object(
            self.json_data, self.key_separator, self.key_filter_callback
        )
        self.assertEqual(flattened, self.flattened_should_be)

    def test_invalid_json(self):
        self.assertRaises(
            LogsterParsingException, self.json_logster.parse_line, '{ "hello": "world"'
        )
