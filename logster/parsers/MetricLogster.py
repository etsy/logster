...
class MetricLogster(LogsterParser):
    def __init__(self, option_string=None):
        ...
        self.count_reg = re.compile(
            ".*METRIC_COUNT\smetric=(?P<count_name>[^\s]+)\s+value=(?P<count_value>[0-9.]+)[^0-9.].*"
        )
        self.time_reg = re.compile(
            ".*METRIC_TIME\smetric=(?P<time_name>[^\s]+)\s+value=(?P<time_value>[0-9.]+)\s*(?P<time_unit>[^\s$]*).*"
        )
    ...
    def parse_line(self, line):
        ...
        if time_match:
            ...
            self.times[time_name]["values"].append(
                float(time_match.groupdict()["time_value"])
            )
    ...
    def get_state(self, duration):
        ...
        for time_name in self.times:
            ...
            metrics.append(
                MetricObject(time_name + ".mean", stats_helper.find_mean(values), unit)
            )
            metrics.append(
                MetricObject(
                    time_name + ".median", stats_helper.find_median(values), unit
                )
            )
            metrics += [
                MetricObject(
