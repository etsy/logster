...
class JsonLogster(LogsterParser):
    ...
    def flatten_object(
        self, node, separator=".", key_filter_callback=None, parent_keys=[]
    ):
        """
        ...
        for key, item in iterator:
            ...
            if type(item) in (list, dict):
                ...
                items.update(
                    self.flatten_object(
                        item, separator, key_filter_callback, parent_keys + [str(key)]
                    )
                )
            else:
    ...
    def parse_line(self, line):
        ...
        self.metrics = self.flatten_object(
            json.loads(line), self.key_separator, self.key_filter
        )
