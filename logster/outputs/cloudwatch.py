        ...
        if self.instance_id is None:
            try:
                ...
                raise CloudWatchException(
                    "Can't connect Amazon meta data server to get InstanceID : (%s)"
                )
    ...
    def get_signed_url(self):
        ...
        try:
            if sys.version_info[:2] == (2, 5):
                signature = hmac.new(
                    key=self.secret_key, msg=string_to_sign, digestmod=hashlib.sha256
                ).digest()
            else:
                signature = hmac.new(
...
class CloudwatchOutput(LogsterOutput):
    ...
    def add_options(cls, parser):
        ...
        parser.add_option(
            "--aws-secret-key",
            action="store",
            default=os.getenv("AWS_SECRET_ACCESS_KEY_ID"),
    ...
    def __init__(self, parser, options, logger):
        ...
        if not options.aws_key or not options.aws_secret_key:
            parser.print_help()
            parser.error(
                "You must supply --aws-key and --aws-secret-key or Set environment variables. AWS_ACCESS_KEY_ID for --aws-key, AWS_SECRET_ACCESS_KEY_ID for --aws-secret-key"
            )
    ...
    def submit(self, metrics):
        for metric in metrics:
            ...
            if not self.dry_run:
                try:
                    cw = CloudWatch(
                        self.aws_key, self.aws_secret_key, metric
                    ).get_instance_id()
