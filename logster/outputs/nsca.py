...
class NSCAOutput(LogsterOutput):
    ...
    def __init__(self, parser, options, logger):
        ...
        if not options.nsca_host:
            parser.print_help()
            parser.error(
                "You must supply --nsca-host when using 'nsca' as an output type."
            )
    ...
    def submit(self, metrics):
        for metric in metrics:
            ...
            nsca_cmd = "echo '%s' | %s -H %s -p %s" % (
                metric_string,
                NSCAOutput.send_nsca,
                self.host[0],
                self.host[1],
            )
